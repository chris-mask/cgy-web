/*
* webSocket
* 初始化websocket方法，创建方法，发送信息方法，接收信息方法，重新连接方法， 关闭websocket方法
* 作者：李馥池
* */
...
function getWebIP(){
  var curIP = process.env.BASE_API;
  curIP = curIP.split('//')[1]
  var url = '';
  ...
  if(process.env.NODE_ENV == 'development'){
    url = "ws://"+ curIP + "/message/api/portalpublic/websocket/" + store.getters.userId + '_' + rdmNum
  }else{
    url = "wss://"+ curIP + "/message/api/portalpublic/websocket/" + store.getters.userId + '_' + rdmNum
  }
...
// const wsUrl = "ws://"+ getWebIP() + "/message/api/portalpublic/websocket/" + store.getters.userId;
const wsUrl = getWebIP();
...
let sendMsg = (data,callback)=>{	//发送数据,接收数据
  ...
  socket.onmessage = ev=>{
    ...
    Notification.info({
      title: '消息',
      message: data.sendUserName + '给您发送了一条信息，请到消息中心查看',
      duration: 10000
    })
    // store.commit('SET_MESSAGE_NEW_TIPS',data)
...
if(store.getters.userId && store.getters.userId != ''){
  createSocket(wsUrl)
}

export default {sendMsg,createSocket,getWebIP,closeConnect}
