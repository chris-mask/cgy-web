      ...
      <div class="list-title">
        ...
        <div class="left">
          ...
          <div class="title-btn-group">
            ...
            <div class="btn" @click="goHandle('add',id)" v-if="allowAdd.twoAllow && powerEdit">新增</div>
            <div class="btn" @click="handleExport" v-if="allowExport.twoAllow">导出</div>
            <div class="btn del" @click="delItemHandle" v-if="allowDelete.twoAllow && powerEdit">删除</div>
          </div>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        allowDetail: true,
        allowDelete: this.authorityControl('SYSTEM:CUSTOMER:MANAGE','SYSTEM:CUSTOMER:CENTER','SYSTEM:CUSTOMER:CENTER:PROPERTY:Certificate:DELETE'),//删除证书
        allowGetList: this.authorityControl('SYSTEM:CUSTOMER:MANAGE','SYSTEM:CUSTOMER:CENTER','SYSTEM:CUSTOMER:CENTER:PROPERTY:Certificate:LIST'),//证书列表
        allowExport: this.authorityControl('SYSTEM:CUSTOMER:MANAGE','SYSTEM:CUSTOMER:CENTER','SYSTEM:CUSTOMER:CENTER:PROPERTY:Certificate:EXPORT'),//导出证书列表
      }
    },
    mounted() {
      this.getList();
        ...
        if(this.allowGetList.twoAllow){
          queryCertificateList(this.listQuery).then(res => {
            if (res.code === 200) {
              ...
              // }
              if(this.id !== undefined) {
                this.$emit('certificateTotal', Number(res.data.total));
              }
            }
          })
        }else{
