  ...
  export default {
    ...
    data() {
      return {
        ...
        allowDetail: true,
        allowDelete: this.authorityControl('SYSTEM:CUSTOMER:MANAGE','SYSTEM:CUSTOMER:CENTER','SYSTEM:CUSTOMER:CENTER:PROPERTY:Trademark:DELETE'),//删除商标
        allowGetList: this.authorityControl('SYSTEM:CUSTOMER:MANAGE','SYSTEM:CUSTOMER:CENTER','SYSTEM:CUSTOMER:CENTER:PROPERTY:Trademark:LIST'),//商标列表
        allowExport: this.authorityControl('SYSTEM:CUSTOMER:MANAGE','SYSTEM:CUSTOMER:CENTER','SYSTEM:CUSTOMER:CENTER:PROPERTY:Trademark:EXPORT'),//导出商标列表
      }
    },
    mounted() {
      this.getDictoryData();
        ...
        if(this.allowGetList.twoAllow){
          queryTrademarkList(this.listQuery).then(res => {
            if (res.code === 200) {
              ...
              if(this.id !== undefined){
                this.$emit('trademarkTotal',Number(res.data.total));
              }
            }
          })
        }else{
