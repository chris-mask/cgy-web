...
<template>
  <div class="notice-form">
   <!-- 锚点按钮 -->
    ...
    <div  class="step" >
        <el-button :class="classChoose==5?'class-Choose':''"   @click="jump('n-info')"   >基本信息</el-button>
        <el-button :class="classChoose==6?'class-Choose':''"   @click="jump('n-subsidies')" >补贴项目信息</el-button>
        <el-button :class="classChoose==7?'class-Choose':''"   @click="jump('n-interpretation')"  >政策解读</el-button>
        <el-button :class="classChoose==8?'class-Choose':''"   @click="jump('n-original')"  >政策原文</el-button>
    ...
    <!--基本信息-->
    <el-form ref="form"  id="n-info"  :rules="rules" :model="form" label-width="auto" label-position="left" @submit.native.prevent>
            ...
            <el-col :span="10" :offset="2">
              <div class="address-bar">
                ...
                <div class="address-group" v-if="!isCheck">
                  <el-select @change='changeProvince' class="address-input" clearable v-model="form.provinceId"
                             placeholder="省份">
                    <el-option v-for="item in province_list" :key="item.dictId" :label="item.dictName"
                               :value="item.dictId"></el-option>
                  </el-select>
                  <el-select @change='changeCity' class="address-input" clearable v-model="form.cityId"
                             placeholder="城市">
                    <el-option v-for="item in city_list" :key="item.dictId" :label="item.dictName"
                               :value="item.dictId"></el-option>
                  </el-select>
                  <el-select @change='changeDistrict' class="address-input" clearable v-model="form.districtId"
                             placeholder="县区">
                    <el-option v-for="item in district_list" :key="item.dictId" :label="item.dictName"
                               :value="item.dictId"></el-option>
                  </el-select>
                  <el-select class="address-input" clearable v-model="form.townId" placeholder="镇街">
...
<!--            </el-col>-->
          ...
          <el-row type="flex" :gutter="10">
            ...
            <el-col :span="10">
              <div class="address-bar" style="padding-left: 0">
                ...
                <div v-else>{{form.subsidyIndustryName}}</div>
              </div>
            </el-col>
          </el-row>
          <el-row type="flex" :gutter="10">
          ...
          <el-row type="flex" :gutter="10">
            <el-col :span="20" :offset="2">
              <el-form-item label="备注：">
                <el-input maxlength="300" v-if="!isCheck" v-model="form.noticeRemark" type="textarea" :rows="3"
    ...
    <!--补贴项目信息-->
    <el-card  id="n-subsidies"   class="top-margin-10 project-massage">
      <div slot="header" class="clearfix">
        <div>
                ...
                <div class="project-form">
                  ...
                  <div class="project-name">
                    ...
                    <div class="right" v-if="!isCheck">
                      <button class="del-project-btn add-project-btn" @click="addSubsidyHandle(item,index)">新增补助
                      </button>
...
<!--                          <span class="xin">*</span>-->
                          最低补助：
                          <el-input v-if="!isCheck" v-model="data.subsidyPriceMin" class="money-input"
                                    placeholder="最低补助" onkeyup="value=value.match(/^\d*(\.?\d{0,2})/g)[0] || null" maxlength="4">
                            <template slot="append">万元</template>
                          </el-input>
                          <div v-else>{{data.subsidyPriceMin&&data.subsidyPriceMin!==''?data.subsidyPriceMin + '万元':0 +
                            '万元'}}
                          </div>
                        </div>
...
<!--                          <span class="xin">*</span>-->
                          最高补助：
                          <el-input v-if="!isCheck" onkeyup="value=value.match(/^\d*(\.?\d{0,2})/g)[0] || null" maxlength="4"
                                    v-model="data.subsidyPriceMax" class="money-input"
                                    placeholder="最高补助">
                            <template slot="append">万元</template>
                          </el-input>

                          <div v-else>{{data.subsidyPriceMax&&data.subsidyPriceMax!==''?data.subsidyPriceMax+ '万元':0+'万元'}}
                          </div>
                        </div>
                      </div>
                      <div class="remarks-form">
                        补助详情：
                        <el-input maxlength="4000" v-if="!isCheck" v-model="data.subsidyDesc" resize="none"
                                  type="textarea"/>
                        <div style="white-space: normal;" v-else>{{data.subsidyDesc}}</div>
                      </div>
                    </div>
                    ...
                    <div class="content">
                      <div class="list" v-for="(list,index) in item.policyApplyConditionVoList">
                        ...
                        <div class="list-body">
                          ...
                          <div class="sub-list">
                            <template
                              v-if="list.policyApplyConditionSqlVoList && list.policyApplyConditionSqlVoList.length !==0">
                              <div class="item" v-for="(childList,num) in list.policyApplyConditionSqlVoList">
                                <div v-if="childList.tabelType" class="left">
                                  <div>#{{num+1}}.分类：{{childList.tabelType}}</div>
                                  <div>数据表：{{childList.zhTableName}}</div>
                                  <div>select字段：{{childList.zhSelectFieldName}}</div>
                                  <div>where字段：{{childList.zhWhereFieldName}}</div>
                                  <div>对比符号：{{childList.compareMark}}</div>
                                  <div>对比值：{{childList.compareValue}}</div>
                                  <div v-if="childList.selectFieldName == '[count]'">统计：是</div>
                                </div>
                                <div v-else class="left">
                                  #{{num+1}}.自定义SQL(支持系统变量)：{{childList.customSql}}
                                </div>
                                <div class="right" v-if="!isCheck">
                                  <button class="btn" @click="editMentHandle(list,index,childList,num)" v-if="allowCondition.twoAllow">编辑</button>
                                  <button class="btn" @click="delMentHandle(list,index,childList,num)" v-if="allowCondition.twoAllow">删除</button>
                                </div>
                              </div>
                            </template>
                          </div>
                  ...
                  <div class="main-declare" style="margin-top: 30px;">
                    <div class="title">
                      ...
                      <button class="add-term-btn" @click.prevent="addMaterialBtnHandle(index,item.projectName)" v-if="!isCheck">新增材料
                      </button>
                    </div>
                      ...
                      <div class="list" v-for="(material,nums) in item.policyApplyMaterialLists">
                        ...
                        <div class="list-body">
                          <div class="term-title" style="align-items: normal;">
                              ...
                              <div style="width: 33%" class="file-model">
                                ...
                                <ul class="files-list">
                                  <li v-for="(childMat,childNum) in material.templateFileIds">
                                    {{childNum+1}}.{{childMat.fileName}}<i
                                    @click="deleteTemplateFile(index,nums,childNum)"
                                    v-if="!isCheck"
                                    class="icon-del-file el-icon-delete"></i>
                                  </li>
                                </ul>
                              </div>
                            </div>
                            ...
                            <div class="title-btn-group">
                              <button class="btn" @click="deleteDetailed(index,nums)" v-if="!isCheck">移除</button>
                            </div>
                          </div>
                        </div>
                      </div>
    ...
    <!--政策解读-->
    <el-card  id="n-interpretation"   class="top-margin-10">
      <div slot="header" class="clearfix">
      ...
      <div>
        <el-row :gutter="12">
          <el-col :span="20" :offset="2">
            <Tinymce v-if="!isCheck" v-model="form.noticeSummary" :height=400 ref="editor"/>
            <p class="editorHtml" v-else v-html="form.noticeSummary">{{form.noticeSummary}}</p>
          </el-col>
        </el-row>
      </div>
    </el-card>
    ...
    <!--原文-->
    <el-card  id="n-original"   class="top-margin-10">
      <div slot="header" class="clearfix">
      ...
      <div>
        <el-row :gutter="12">
          <el-col :span="20" :offset="2">
            <Tinymce v-if="!isCheck" v-model="form.noticeContent" :height=400 ref="editor2"/>
            <p class="editorHtml" v-else v-html="form.noticeContent">{{form.noticeContent}}</p>
          </el-col>
        </el-row>
        <el-row :gutter="12" class="top-margin-10">
          <el-col :span="20" :offset="2">
            <el-form-item label="原文链接：">
              ...
              <input v-if="!isCheck" v-model="form.sourceUrl" type="text" placeholder="请输入原文链接"
    ...
    <div class="center-items top-margin-30">
      <el-form-item>
        <el-button type="primary" v-if="isCreate" @click="noticeHandSubmit('form')" v-preventReClick>确认</el-button>
        <el-button type="primary" v-if="isEdit" v-else @click="noticeHandSubmit('form')" v-preventReClick>修改</el-button>
      ...
      <el-form :model="addTermParams" :rules="termParamsRules" ref="termParamsRules" label-width="88px">
        ...
        <div v-if="addTermParams.autoMatch == 1" v-for="(item, index) in conditionVarList">
          <el-form-item label="条件表达：" style="white-space: nowrap;" v-if="index === 0">
            ...
            <el-select remote :remote-method="newProjectListMethod" multiple style="width: 150px;margin-left: 10px;" filterable
                       v-loadMore="newLoadMore" :loading="loading" v-if="item.varId == 798613854857859072" @change="newProjectListChange"
    ...
    <!--新增材料--弹窗-->
    <el-dialog title="新增材料" class="editorUserModal ment-modal" :visible.sync="materialModal" width="600px" v-dialogDrag>
      <el-form :model="materialParams" :rules="materialRule" ref="materialForm" label-width="88px">
        <el-form-item style="white-space: nowrap;" class="modal-radio" label="附件分类：" prop="materialType">
          <el-select v-model="materialParams.materialType" clearable>
            <el-option
        ...
        <el-form-item class="modal-radio" label="要求：" prop="materialDesc">
          <el-input maxlength="500" v-model="materialParams.materialDesc" type="textarea" resize="none" :rows="3"
                    placeholder="请输入要求"></el-input>
        </el-form-item>

        <el-form-item class="modal-radio" label="" style="margin-bottom: 0;">
          <div style="width: 88px;color: rgba(68, 68, 68, 0.75)"><span style="color:#F56C6C;margin-right: 4px;">　</span>模板：</div>
          <el-upload
            ...
            action=""
          >
            <el-button class="set-file-btn">
              上传附件
            </el-button>
          </el-upload>
          <p style="margin-bottom: 10px;font-size:12px;color:#ff3f4b;margin-left:10px;line-height: normal">限制文件不超过{{$store.getters.singleCapacity / 1024 + 'M'}}</p>
        </el-form-item>

        <!--文件列表-->
        <el-form-item class="modal-radio" label="　">
          <ul>
            <li v-for="(item,index) in templateFileList">
              {{item.fileName}} <i @click="delTemplateFile(index)" class="del-files el-icon-delete"></i>
            </li>
          </ul>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="setBtn" @click="setAddMaterial">确 定</el-button>
        <el-button class="cananl" @click="cananlAddMaterial">取 消</el-button>
      </span>
    </el-dialog>
...
<script>
  ...
  import {commonDict, commonDictOfRegion, commonDictGetNameByCaChe} from '@/api/utils'
  ...
  import {policyUpload, policyProjectUpload} from '@/utils/uploadPathMethods'
  ...
  export default {
    ...
    data() {
      return {
        ...
        //新增材料--点击新增获取当前父级的值
        getMaterialParentIndex: '',
        //新增材料--点击新增获取当前项目名
        getMaterialprojectName: '',
        keyWord: '',
        //吸顶
        fixed:false,
        //二维码是否显示
    ...
    methods: {
      ...
      //   // let href = 'http://192.168.1.19:8080/#/policy/detail?noticeId='+this.$route.query.noticeId+'&userId='+this.$store.state.user.userId;
              ...
              this.select_projects.forEach(item => {
                if (item.projectId === this.addTermParams.projectId) {
                  if (this.addTermParams.autoMatch == 1) {  // 人工匹配没有条件
                    ...
                    // console.log(this.conditionVarList,"===setAddTerm新增==conditionVarList======>")
      ...
      editTermHandle(data, index) {
        ...
        // console.log(this.conditionVarList,"===editTermHandle==conditionVarList======>")
          setTimeout(() => {
      ...
      handlePoliceUpload(option) {
        ...
        let filePath = policyUpload(this.form.provinceName, this.form.cityName,this.form.publishDate, fileName) + '.' + fileType
      ...
      noticeHandSubmit(formName) {
        this.$refs[formName].validate((valid, error) => {
          if (valid) {
                ...
                if (this.select_projects[i] !== undefined && this.select_projects[i].policyNoticeProjectRelList.length !== 0 && this.select_projects[i].policyNoticeProjectRelList.length > 1) {
                  for (let j = 0; j <= this.select_projects[i].policyNoticeProjectRelList.length; j++) {
                    ...
                    if (this.select_projects[i].policyNoticeProjectRelList[j + 1] !== undefined) {
                      if (this.select_projects[i].policyNoticeProjectRelList[j].projectLevel === this.select_projects[i].policyNoticeProjectRelList[j + 1].projectLevel) {
                        this.$message.warning(`${this.select_projects[i].projectName} 中存在相同的项目级别，请重新选择`);
                        return
                      }
                    }
                  }
            ...
            // if (this.select_projects.length !== 0) {
            //   this.select_projects.forEach(item=>{
            //     if(item.policyApplyMaterialLists.length !== 0){
            //       item.policyApplyMaterialLists.forEach(data=>{
            //         if(data.templateFileIds.length == 0){
            //           this.$message.warning(`${item.projectName} 中的材料清单，请选择模板文件`);
            //           throw new Error()
            //         }
            //       })
            //     }
            ...
            this.select_projects.forEach(item => {
              item.policyNoticeProjectRelList.forEach(data => {
                ...
                if (data.subsidyPriceMin == '' || data.subsidyPriceMin == null || data.subsidyPriceMin == undefined) {
                  data.subsidyPriceMin = 0
                }


                if (data.subsidyPriceMax == '' || data.subsidyPriceMax == null || data.subsidyPriceMax == undefined) {
                  data.subsidyPriceMax = 0
                }

                if (Number(data.subsidyPriceMin) > Number(data.subsidyPriceMax)) {
                  this.$message.warning(`${item.projectName} 中最低补助不能大于最高补助，请重新输入`);
                  throw new Error()
                }

                // data.subsidyPriceMin = data.subsidyPriceMin * 10000
                // data.subsidyPriceMax = data.subsidyPriceMax * 10000
                ...
                let obj = {
                  ...
                  subsidyPriceMin: data.subsidyPriceMin * 10000,
                  subsidyPriceMax: data.subsidyPriceMax * 10000,
                  subsidyId: data.subsidyId ? data.subsidyId : "",
                };
                selectArr.push(obj)
              })
              ...
              //材料清单--数据重组
              if (item.policyApplyMaterialLists && item.policyApplyMaterialLists.length !== 0) {
                let materialLists = cloneDeep(item.policyApplyMaterialLists)

                materialLists.forEach(item => {
                  if (item.templateFileIds.length !== 0) {
                    let fileListArr = item.templateFileIds.map(data => {
                      return data.fileId;
                    })
                    item.templateFileIds = fileListArr.join(',');
                    material.push(item);
                  }
                })
              }
            ...
            this.form.policyApplyMaterialLists = material;
      ...
      //点击--新增材料
      addMaterialBtnHandle(index,projectName) {
        this.materialModal = true;
        this.getMaterialParentIndex = index;
        this.getMaterialprojectName = projectName;
        this.getAnnexTypeData();
      },
      ...
      //新增材料--上传模板
      handleTemplateUpload(option) {
        if(!this.form.provinceId || this.form.provinceId == ''){
          this.$message.error('请先选择省');
          this.fileList = JSON.parse(JSON.stringify(this.fileList))
          return;
        }
        if(!this.getMaterialprojectName || this.getMaterialprojectName == ''){
          this.$message.error('项目不存在，请检查');
          this.fileList = JSON.parse(JSON.stringify(this.fileList))
          return;
        }
        if (option.file.name.length >= 150) {
        ...
        let fileName = option.file.name.substring(0,index);
        let fileType = option.file.name.substring(index+1,option.file.name.length);
        let filePath = policyProjectUpload(this.form.provinceName, this.form.cityName,this.getMaterialprojectName, fileName) + '.' + fileType
        uploadFilesOss(filePath, option.file, option).then(res => {
          if (res) {
            let FileList = [];
            let addFile = {
            ...
            addFileBatch(params).then(response => {
              if (response.code == 200) {
                this.$message.success('上传成功');
                this.templateFileList.push(response.data[0]);
              }else{
                this.$message.error('上传失败，请检查重新上传');
                this.fileList = JSON.parse(JSON.stringify(this.fileList))
              }
      ...
      //新增材料--删除模板
      delTemplateFile(index) {
        this.templateFileList.splice(index, 1);
      },

      //新增材料--确定新增材料
      setAddMaterial() {
        this.$refs.materialForm.validate((valid, error) => {
          if (valid) {
            // if (this.templateFileList.length == 0) {
            //   this.$message.warning('请选择模板');
            //   return
            // }
            this.materialParams.templateFileIds = this.templateFileList;

            this.annexTypeData.forEach(item => {
              if (item.dictId == this.materialParams.materialType) {
                this.materialParams.materialTypeName = item.dictName
              }
            });


            this.materialParams.projectId = this.select_projects[this.getMaterialParentIndex].projectId;

            this.select_projects[this.getMaterialParentIndex].policyApplyMaterialLists.push(this.materialParams);
      ...
      //材料清单--删除模板文件
      deleteTemplateFile(index, nums, childNum) {
        this.select_projects[index].policyApplyMaterialLists[nums].templateFileIds.splice(childNum, 1);
      },

      //材料清单--移除清单
      deleteDetailed(index, nums) {
        this.select_projects[index].policyApplyMaterialLists.splice(nums, 1);
        this.select_projects = cloneDeep(this.select_projects);
      },
      ...
      getComparisonNum (varId,num){
        ...
        getComparisonByVarId({varId: varId}).then(res =>{
          if (res.code === 200){
            // if(varId == 798613854857859072){
            //   this.comparisonList[num]= [];
            //   this.$set(this.comparisonList,num,[]);
            //   // console.log(num,"===>")
            // }else{
        ...
        if (this.$route.query.is_edit || this.$route.query.is_check) {
          if (JSON.stringify(newVal) !== '{}') {
            if (this.noticeType == 1) {
              ...
              // 数据重组
              this.form.policyNoticeProjectSubsidyRelVos.forEach(item => {
                // 项目级别数据重组
                if (item.policyNoticeProjectSubsidyVos.length !== 0) {
                  item.policyNoticeProjectSubsidyVos.forEach(data => {
                    data.projectId = item.projectId;
                    if (data.subsidyPriceMin && data.subsidyPriceMin !== '' && data.subsidyPriceMin > 1000) {
                      // data.subsidyPriceMin = data.subsidyPriceMin.toString();
                      // data.subsidyPriceMin = data.subsidyPriceMin / 10000
                      this.$set(data,'subsidyPriceMin',data.subsidyPriceMin / 10000)
                    }
                    if (data.subsidyPriceMax && data.subsidyPriceMax !== '' && data.subsidyPriceMax > 1000) {
                      // data.subsidyPriceMax = data.subsidyPriceMax.toString();
                      // data.subsidyPriceMax = data.subsidyPriceMax / 10000
                      this.$set(data,'subsidyPriceMax',data.subsidyPriceMax / 10000)
                    }
                  })
                  if (item.policyNoticeProjectSubsidyVos.length > 1) {
                    item.isDel = true;
                  }
                  this.$set(item,'policyNoticeProjectRelList',item.policyNoticeProjectSubsidyVos)
                  // item.policyNoticeProjectRelList = item.policyNoticeProjectSubsidyVos;
                } else {
                  this.$set(item, 'policyNoticeProjectSubsidyVos', [])
                }
              ...
              this.select_projects = this.form.policyNoticeProjectSubsidyRelVos;
              this.select_projects.forEach(item => {
              ...
              //当页面是详情页的时候，接口传id,获取中文名
              if (this.$route.query.is_check) {
                //有效期转换
                this.termDateName = `${this.form.validDateBegin} 至 ${this.form.validDateEnd}`
              }
