  ...
  <div class="app-container">
    <el-card class="box-card box-margin">
      <div slot="header" class="clearfix headerTop">
        <span>{{parentId != undefined ? '添加子角色' : roleId =='' || roleId == undefined ? '新建角色' : '编辑角色'}}</span>
      </div>
      <div class="addUserContent">
        <el-form ref="roleForm" :model="roleForm" :rules="rules" class="demo-ruleForm">
          <el-row>
            <el-col :span="12">
              <el-form-item label="角色名称:" prop="roleName">
                <el-input size='small' v-model="roleForm.roleName" placeholder="请输入角色名称" maxlength="45"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="12">
              <el-form-item label="用户:" prop="addUserRoleDtos">
                <div class="userCont">
                  <div class="userList">
                    <div class="userBox" v-for="(item,index) in roleForm.addUserRoleDtos"
                         v-if="roleForm.addUserRoleDtos != ''">
                      <span>{{item.userName}}</span>
                      <i class="el-icon-close" @click="deleteUser(item,index)"></i>
                    </div>
                    <div class="userAddBtn" @click="openUserList">
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="数据权限:" prop="dataPermissionsType">
                <el-select size='small' v-model="roleForm.dataPermissionsType" clearable placeholder="请选择" @change="changeDataType"
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="允许查看的项目类型:">
                <el-checkbox-group v-model="checkedList" @change="handleCheckedCitiesChange">
                  <el-checkbox v-for="item in projectTypeList" :label="item.dictId" :key="item.dictId">{{item.dictName}}</el-checkbox>
                </el-checkbox-group>
              </el-form-item>
            </el-col>
          </el-row>
...
<!--                </el-input>-->
                <EditorText :id="editor1" :value="roleForm.roleDesc" ref="editor" @changeValue="changeValue($event)"></EditorText>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
    ...
    <div class="btnList">
      <el-button size='small' type="primary" @click="addRoleInfo" v-if="roleId == undefined || roleId == '' && allowAdd.twoAllow ">确定</el-button>
      <el-button size='small' type="primary" @click="editRoleBox" v-if="roleId != undefined && roleId != '' && allowUpdate.twoAllow ">修改</el-button>
    ...
    >
      <div class="userCheckbox">
        <div class="dialogChoose" style="display: flex;">
          <el-select style="margin-right: 10px;" v-model="deptIds" multiple size="small" filterable clearable placeholder="请选择部门" @change="deptChange">
            <el-option
        ...
        <el-checkbox-group v-model="checkList" @change="handleCheckedUserChange">
          <el-checkbox v-for="item in userList" :label="item.userId" :key="item.userId" @change="changeCheckUser(item)" :checked="item.checked">
            {{item.userName}}
          </el-checkbox>
        </el-checkbox-group>
      </div>
    ...
    methods: {
      ...
      addRoleInfo() {
        ...
        const params = {
          ...
          // orderTypeRoleDto: orderTypeRoleDto
        }

        if(this.checkedList != '' && this.checkedList.length != 0){
          let orderTypeRoleDto = {
            companyId: this.$store.getters.companyId
          };

          orderTypeRoleDto.orderType = this.checkedList.join(',')
          params.orderTypeRoleDto = orderTypeRoleDto;
        }
      ...
      editRoleBox() {
        ...
        this.roleForm.addRolePermissionDtos.forEach(item=>{
          item.selected = true
        })
        ...
        if(this.checkedList != '' && this.checkedList.length != 0){
          ...
          if(this.roleForm.orderTypeRoleVo){
            orderTypeRoleDto.sid = this.roleForm.orderTypeRoleVo.sid
          }
        ...
        queryRoleInfoById(params).then(res => {
          if (res.code == 200) {
            this.roleForm = {
              ...
              orderTypeRoleVo: res.data.orderTypeRoleVo
            }
            ...
            if(res.data.orderTypeRoleDto){
              this.checkedList = [];
              if(res.data.orderTypeRoleVo && res.data.orderTypeRoleVo.orderType){
                this.checkedList = res.data.orderTypeRoleVo.orderType.split(',')
              }
            }
