    ...
    <el-card class="box-card box-margin">
      ...
      <div slot="header" class="clearfix headerTop">
        <div class="headerLeft">
          <el-input size='small' class="searchInput" v-model="dictName" placeholder="字典名称" @keyup.enter.native="goSearch" clearable></el-input>
          <el-button size='small' type="primary" icon="el-icon-search" @click="goSearch">搜索</el-button>
        </div>
        <div class="headerRig">
          <el-button size='small' type="primary" @click="openAddDialog" v-if="!queryCompany && allowGetTwoAdd.threeAllow">新建字典</el-button>
        </div>
      </div>
        ...
        <el-tab-pane v-for="(item,index) in tabList" :label="item.label" :name="item.name" :key="index">
          <div class="materielTable">
            <el-table :data="dictionariesList" v-loading="listLoading" border fit highlight-current-row
              ...
              <el-table-column align="center" min-width="160px" label="字典名称" sortable="custom" prop="dictName">
              ...
              <el-table-column align="center" label="操作" width="270px" class-name="small-padding fixed-width" v-if="index == 0">
                <template slot-scope="scope">
                  <span class="spanButton col_skyblue" v-if="scope.row.parentId = '0' && allowAdd.threeAllow " @click="openAddDialog(scope.row)">添加子字典</span>
                  ...
                  <el-popconfirm
                    ...
                    iconColor="red"
                    title="是否确定删除？"
                    @onConfirm="deleteDisction(scope.row)"
              ...
              <el-table-column align="center" label="操作" width="350px" class-name="small-padding fixed-width" v-if="index == 1">
                <template slot-scope="scope">
                  <span class="spanButton col_skyblue" v-if="scope.row.parentId = '0' && allowGetTwoAdd.threeAllow " @click="openAddDialog(scope.row)">添加子字典</span>
    ...
    >
      <el-form :model="addDictForm" :rules="rules" ref="addDictForm" label-width="100px">
        <el-form-item label="字典名称:" prop="dictName">
          <el-input size='small' v-model="addDictForm.dictName" v-if="!isShowDetail" placeholder="请输入字典名称"></el-input>
        ...
        <el-form-item label="标识符" prop="dictCode" v-if="queryCompany == false">
          <el-input size='small' v-model="addDictForm.dictCode" v-if="!isShowDetail" placeholder="请输入标识符"></el-input>
          <span v-else>{{addDictForm.dictCode}}</span>
        </el-form-item>


        <el-form-item label="值:" prop="dictValue">
          <el-input size='small' v-model="addDictForm.dictValue" v-if="!isShowDetail" placeholder="请输入字典值"></el-input>
        ...
        <el-form-item label="允许多少级:" prop="allowLevel">
          <el-input size='small' @keyup.native="levelKeyUp" v-model="addDictForm.allowLevel" v-if="!isShowDetail" type="number" placeholder="请填数字"></el-input>
          <span v-else>{{addDictForm.allowLevel}}</span>
        </el-form-item>
        <el-form-item label="备注:" prop="dictDesc">
<!--          <el-input size='small' type="textarea" v-model="addDictForm.dictDesc" :disabled="isChecked" v-if="!isShowDetail" placeholder="请输入"></el-input>-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          dictScope: [{required: true, message: '请选择字典范围', trigger: 'blur'}],
        ...
        isShowDetail: true,
        editor1:'editor1',
        allowGetList: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:CUSTOM', 'SYSTEM:DICT:CUSTOM:LIST'),
        allowAdd: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:CUSTOM', 'SYSTEM:DICT:CUSTOM:INSERT'),
        allowUpDown: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:CUSTOM', 'SYSTEM:DICT:CUSTOM:UPORDOWN'),
        allowDelete: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:CUSTOM', 'SYSTEM:DICT:CUSTOM:DELETE'),
        allowDetail: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:CUSTOM', 'SYSTEM:DICT:CUSTOM:DETAILS'),
        allowUpdate: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:CUSTOM', 'SYSTEM:DICT:CUSTOM:EDIT'),

        allowGetTwoList: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:SYSTEM', 'SYSTEM:DICT:SYSTEM:LIST'),
        allowGetTwoAdd: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:SYSTEM', 'SYSTEM:DICT:SYSTEM:INSERT'),
        allowGetTwoUpDown: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:SYSTEM', 'SYSTEM:DICT:SYSTEM:UPORDOWN'),
        allowGetTwoDelete: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:SYSTEM', 'SYSTEM:DICT:SYSTEM:DELETE'),
        allowGetTwoDetail: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:SYSTEM', 'SYSTEM:DICT:SYSTEM:DETAILS'),
        allowGetTwoUpdate: this.authorityControl('SYSTEM:BACKGROUND:MANAGE', 'SYSTEM:DICT:MANAGE', 'SYSTEM:DICT:SYSTEM', 'SYSTEM:DICT:SYSTEM:EDIT'),
      }
    },
    activated() {
    ...
    methods: {
      ...
      getDictList() {
        ...
        queryDictInfoList(params).then(res => {
          if (res.code == 200) {
        ...
        queryDictInfoList(params).then(res => {
          if (res.code == 200) {
            if (res.data.data.length === 0) {
              this.$message.warning('当前字典没有子字典，请先进行添加')
            } else {
              res.data.data.forEach(item => item.childs = 'childs');
              row.children = res.data.data;
              row.children.forEach(item => {
                item.fatherId = row.dictId
              })
            }
      ...
      dataUpDown(row, status) {
        ...
        goUpOrDownAssets(params).then(res => {
          // goUpOrDownAssets({a:enc}).then(res => {
          if (res.code == 200) {
      ...
      //级数限制输入数字
      levelKeyUp() {
        this.addDictForm.allowLevel = this.addDictForm.allowLevel.replace(/[^\.\d]/g, '');
        this.addDictForm.allowLevel = this.addDictForm.allowLevel.replace('.', '');
      }
    }
  }
</script>
