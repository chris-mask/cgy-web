  ...
  <div class="app-container">
    <el-card class="box-card">
      <el-row type="flex" justify="space-between" class="filterList">
        <div class="df">
          <el-input v-model="variableQuery.varName" @keyup.enter.native="handleSearch" clearable placeholder="请输入变量名称"></el-input>
          <el-button class="filter-item bg_blue" icon="el-icon-search" @click="handleSearch">搜索</el-button>
        </div>
        <div class="df">
          <el-button class="filter-item bg_blue" @click="goAdd" v-if="allowAdd.twoAllow">新增变量</el-button>
        </div>
      </el-row>
    </el-card>
    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column label="变量值">
          <template slot-scope="scope">
            <span>{{ scope.row.varValue }}</span>
          </template>
        </el-table-column>
        <el-table-column label="备注">
          <template slot-scope="scope">
            <span>{{ scope.row.varDesc }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <span class="spanButton" @click="goEdit(scope.row.varId)" v-if="allowUpdate.twoAllow">编辑</span>
            <span class="shu">|</span>
            <span class="spanButton col_red" @click="deleteFlow(scope.row.varId)" v-if="allowDelete.twoAllow">删除</span>
          </template>
        </el-table-column>
      </el-table>
      <div class="pagination-container">
        <div class="totalText">
          <span>总数：{{ total }}</span>
          <span>总页数:{{ Math.ceil(total / variableQuery.pageSize) || Math.ceil(total / 10) }}</span>
          <span>当前页：{{ variableQuery.page || 1 }}</span>
        </div>
        <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
                       :current-page="currentPage" :page-sizes="[10,20,30, 50]" :page-size="variableQuery.pageSize"
...
<script>
  import {queryVariableList, deleteVariable} from '@/api/conditionVariable'
  ...
  export default {
    ...
    methods: {
      ...
      //搜索按钮
      handleSearch() {
        this.variableQuery.page = 0;
        this.variableQuery.pageSize = 10;
        this.getVariableList()
      },
      ...
      getVariableList(){
        ...
        queryVariableList(this.variableQuery).then(res=>{
          if(res.code == 200){
      ...
      //删除变量
      deleteFlow(id){
        this.$confirm('是否确定删除此变量', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          deleteVariable({varId: id}).then(res=>{
            if(res.code == 200){
      ...
      handleSizeChange(val) {
        this.variableQuery.pageSize = val
        this.getVariableList();
      },

      // 分页 -- 改变当前页码
      handleCurrentChange(val) {
        this.variableQuery.page = (val - 1) * this.variableQuery.pageSize
        this.getVariableList();
      },
    }
  }
