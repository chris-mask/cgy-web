          ...
          <el-row>
            <el-col>
              <div class="conditionBox">
                ...
                <el-select size="small" filterable v-model="singnForm.signCompanyId" placeholder="签约公司"
                ...
                <el-select filterable size="small"  v-model="singnForm.signUserId" placeholder="选择签订人"
      ...
      <el-card class="box-card cardCont">
        ...
        <div class="projectTable">
          <el-table :data="signReport" border fit highlight-current-row ref="signReport">
            ...
            <el-table-column label="签单数量" align="center" min-width="140">
              <template slot-scope="scope">
                <span>{{scope.row.signCount}}</span>
              </template>
            </el-table-column>
            <el-table-column label="高企认定复审数" align="center" min-width="140">
              <template slot-scope="scope">
                <span>{{scope.row.companyAffirmCount}}</span>
              </template>
            </el-table-column>
            <el-table-column label="财务审计数" align="center" min-width="140">
              <template slot-scope="scope">
                <span>{{scope.row.financeCount}}</span>
              </template>
            </el-table-column>
            <el-table-column label="知识产权数" align="center" min-width="140">
              <template slot-scope="scope">
                <span>{{scope.row.intellectualCount}}</span>
              </template>
            </el-table-column>
            <el-table-column label="其他重点项目数" align="center" min-width="140">
              <template slot-scope="scope">
                <span>{{scope.row.otherProjectCount}}</span>
              </template>
            </el-table-column>

            <el-table-column label="立项数" align="center" min-width="140">
              <template slot-scope="scope">
  ...
  export default {
    ...
    methods:{
      ...
      showSignOrder(){
        ...
        querySignOrderStatistical(this.singnForm).then(res=>{
          if(res.code == 200){
      ...
      handleExport(){
        ...
        querySignOrderStatisticalExportForExcel(this.singnForm).then(res=>{
          if(res.code == 200){
            var parData = {
            ...
            downloadCrm(parData).then(response => {
              ...
              const fileName = date + '-签单统计表' + '.xlsx'
              a.href = url
              a.download = fileName
              a.click()
      ...
      signCompanyChange(val) {
        if (val) {
          ...
          // this.selectConditionsList.signCompanyId = '';
          // console.log(this.selectConditionsList,'====this.selectConditionsList===============>')
        }
        ...
        if (val.length != 0) {
          ...
          this.userList.forEach(item => {
              if (item.userId == val) {
                // arr.push(item.userName)
                // const string = arr.join('，')
                this.$set(this.selectConditionsList, 'signUserId', `签订人 - ${item.userName}`)
              }
