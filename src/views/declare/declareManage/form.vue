  ...
  <div class="sale-plan-detail">
      ...
      <div class="detail">
        <div class="name">
          ...
          <el-input class="input"
                    ...
                    :disabled="isEdit"
          />
          <img :src="editIcon" v-if="isEdit" @click="editBtnHandle"/>
        </div>
        <div class="user-type">
          <span class="user-title">服务机构：</span>
          <el-input class="input" placeholder="请输入服务机构" v-model="form.serviceCompany" :disabled="isEdit"/>
        </div>
      </div>
    </div>
        ...
        <el-row class="form-body">
          ...
          <el-col :offset="1" :xs="12" :sm="8" :md="8" :lg="6" :xl="5">
            <div class="item">
              ...
              <el-input :disabled="isEdit" placeholder="请输入申报人" class="input" v-model.trim="form.applyPersonnel"
                        clearable/>
            </div>
          </el-col>
          <!--申报时间-->
          ...
          <el-col :offset="1" :xs="12" :sm="8" :md="8" :lg="6" :xl="5">
            <div class="item">
              ...
              <el-select v-model="form.approvalState" class="input-select" placeholder="请选择立项情况" :disabled="isEdit">
          ...
          <el-col :offset="1" :xs="12" :sm="8" :md="8" :lg="6" :xl="5">
            <div class="item">
              ...
              <el-select v-model="form.recheckState" class="input-select" placeholder="请选择需要验收" :disabled="isEdit">
                <el-option
    ...
    <div class="tab-list">
      ...
      <div class="tab-content">
        <!--进度跟踪-->
        <div v-if="tabAcitive == 0" :class="!isEdit?'tracking-tab':'tracking-tab cur'">

          <div class="box-group"
               v-for="(item,index) in processData"
               @click.stop="!isEdit && processBtnhandle(false,item,index)"
          >
            <div class="box">
              <div class="box-title">
                <div class="num">{{index+1}}</div>
                <div class="title-name">{{item.processName}}</div>
                <i v-if="!isEdit" @click.stop="clearBoxHandle(index,item.processId)"
                   class="icon-clear-box el-icon-circle-close"></i>
              </div>
              <div class="box-content">
                <div>时间：<span>{{item.processDate | dateCut}}</span></div>
                <div class="remark">
          ...
          <div class="box-group" v-if="!isEdit">
            <div class="box">
        ...
        <!--项目概况-->
        <div v-if="tabAcitive == 1" class="project-area-tab">
          <el-input
            type="textarea"
            :rows="2"
          ...
          <div class="tab-content">
            <div class="item" v-for="(item,index) in policeFileList">
              <div class="files-name">
                ...
                <img class="down-img" v-if="isEdit" :src="downImg" @click="downFileHandle(item)"/>
                <button class="files-del-btn" v-if="!isEdit" @click="delPoliceFileHandle(index)">删除</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="tab-content">
            <div class="item" v-for="(item,index) in declareFileList">
              <div class="files-name">
                ...
                <img class="down-img" v-if="isEdit" :src="downImg" @click="downFileHandle(item)"/>
                <button class="files-del-btn" v-if="!isEdit" @click="delDeclareFileHandle(index)">删除</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="tab-content">
            <div class="item" v-for="(item,index) in checkFileList">
              <div class="files-name">
                申报材料： <span class="name">{{item.fileName}}</span>
                <img class="down-img" v-if="isEdit" :src="downImg" @click="downFileHandle(item)"/>
                <button class="files-del-btn" v-if="!isEdit" @click="delCheckFileHandle(index)">删除</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="goPlanList">
      <el-button class="blue btn" v-if="!isEdit" @click="addHandle">保存</el-button>
      <el-button class="btn" @click="goPlanListHandle" v-if="!isEdit">取消</el-button>
      <el-button class="btn" v-if="isEdit" @click="goPlanListHandle">返回</el-button>
    </div>
    ...
    <el-dialog title="新增进度" :visible.sync="updateVisible" class="editorUserModal" width="600px" v-dialogDrag>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="setBtn" @click="addProcessHandle" v-preventReClick>确 定</el-button>
...
<script>
  ...
  import {
    ...
    updateProjectApply,
    //申报管理--详情
    queryProjectApplyDetailById,
    //申报进度--编辑项目
    updateProjectApplyProcess,
    //申报进度--详情
    queryProjectApplyProcessDetailById,
    //申报进度--删除
    deleteProjectApplyProcessByIds
  } from '@/api/declare'
  ...
  import downImg from '../../../../static/down-img.png'
  ...
  export default {
    ...
    methods: {
      ...
      hasPageType() {
        //判断缓存里面是否有decType
        if (localStorage.getItem('decType')) {
          let type = localStorage.getItem('decType');
      ...
      addHandle() {
        ...
        if (this.form.onlineDateEnd === '') {
          this.$message.warning('系统提交截止时间不能为空');
          return
        }
        if (this.form.paperDateEnd === '') {
          this.$message.warning('纸质提交截止时间不能为空');
          return
        }
        if (this.form.subsidyAmount === '') {
          this.$message.warning('预计扶持金额不能为空');
          return
        }

        // this.form.processDtoList = this.processData;
        ...
        delete this.form.processVos;

        //调用编辑接口
        updateProjectApply(this.form).then(res => {
          if (res.code === 200) {
      ...
      //申报管理--获取详情
      getDeclareDetail() {
        queryProjectApplyDetailById({applyId: this.$route.query.id}).then(res => {
          if (res.code === 200) {
            this.form = JSON.parse(JSON.stringify(res.data));
            this.declareFileList = this.form.applyFileUrls;
            this.checkFileList = this.form.recheckFileUrls;
            this.policeFileList = this.form.sourceFileUrls;
            if (res.data.processVos.length > 0) {
              this.processData = res.data.processVos.map(item => {
                item.processDate = item.processDate.slice(0, 10);
                return item;
              });
            } else {
              this.processData = res.data.processVos;
            }
          }
        })
      },
      ...
      //申报进度--删除数据
      clearBoxHandle(index, id) {
        if (localStorage.getItem('editDeclare')) {
          this.processIds.push(id);
          deleteProjectApplyProcessByIds(this.processIds).then(res => {
            if (res.code === 200) {
              this.processIds = [];
              this.getDeclareDetail();
      ...
      //下载文件
      downFileHandle(data) {
        let url = data.fileUrl.replace(/http/,'https')
        window.open(url)
      }
    },
